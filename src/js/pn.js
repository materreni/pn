...
function errorModal(lang, mode, mimeCodec){
	...
	const texts = {
		...
		'browser' : {
			'en' : 'Your browser doesn’t seem to support the video technology used in this work (namely the Media Source API). Try with a desktop browser (<strong>Firefox</strong>, <strong>Chrome</strong>, <strong>Edge</strong> or <strong>Safari</strong>) or any phone with an <strong>Android operating system</strong>.',
			'es' : 'Es posible que tu navegador no sea compatible con la tecnología de video usada por este sitio (especificamente la API Media Source). Probá con un navegador en tu computadora de escritorio (<strong>Firefox</strong>, <strong>Chrome</strong>, <strong>Edge</strong> o <strong>Safari</strong>) o cualquier teléfono con <strong>sistema operativo Android</strong>.'
		},
		'loading'	: {
